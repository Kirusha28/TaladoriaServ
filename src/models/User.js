...
class User {
	...
	static async getUserById(userId) {
		...
		try {
            ...
            SELECT 
                ...
                u.coop_voice_minutes,
                u.solo_voice_minutes,
                u.mmo_voice_minutes,
                u.week_voice_minutes,
                u.message_count,
                u.events_part,
                u.register_date,
                COALESCE(
	...
	static async getUserByIdOrUsername(userIdOrUserName) {
		// 1. Проверка входного параметра до начала работы с БД
		if (userIdOrUserName === undefined || userIdOrUserName === null) {
			console.error(
				"[User.getUserByIdOrUsername] ERROR: User identifier is undefined or null",
			);
			throw new Error("User identifier must be provided");
		}

		try {
			// console.log(
			// 	`[User.getUserByIdOrUsername] Fetching data for: ${userIdOrUserName}`,
			// );

			// Ищем пользователя либо по user_id, либо по username
			const query = `
            SELECT 
                u.user_id,
                u.nickname,
        ...
        `;
			...
			const [rows] = await db.execute(query, [userIdOrUserName, userIdOrUserName]);

			if (!rows || rows.length === 0) {
				console.warn(
					`[User.getUserByIdOrUsername] No user found with identifier: ${userIdOrUserName}`,
				);
				return null;
			}
			...
			if (typeof user.achievements === "string") {
				try {
					...
					console.error(
						"[User.getUserByIdOrUsername] JSON Parse Error (achievements):",
						parseError.message,
					);
					user.achievements = [];
				}
			...
			for (const field of roleFields) {
				if (typeof user[field] === "string") {
					try {
						...
						console.error(
							`[User.getUserByIdOrUsername] JSON Parse Error (${field}):`,
							e.message,
						);
						user[field] = null;
					}
			...
			console.error("--- DATABASE ERROR ---");
			console.error("Method: getUserWithAchievements");
			console.error("Target User ID:", userIdOrUserName);
			console.error("Message:", error.message);
			console.error("SQL State:", error.sqlState);
			console.error("Error Code:", error.code);
			console.error("----------------------");
