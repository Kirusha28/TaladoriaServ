...
const googleCallback = async (req, res) => {
  // Passport.js уже аутентифицировал пользователя и прикрепил его к req.user
  if (req.user) {
    res.redirect(`/auth/success?token=${generateToken(req.user._id)}`); // Перенаправляем на страницу успеха с токеном
  } else {
    res.redirect('/auth/failure'); // Перенаправляем на страницу неудачи
  }
};
